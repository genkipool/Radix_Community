...
export function buildResizableImageHtml(src: string, alt = '') {
    ...
    <div class="editor-img-wrap relative group inline-block max-w-full my-4" style="line-height: 0;">
      ...
      <div class="img-size-toolbar absolute top-4 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1 bg-black/80 backdrop-blur-md p-1.5 rounded-xl border border-white/20 shadow-2xl z-10 scale-90 group-hover:scale-100 pointer-events-auto">
        <button type="button" class="img-size-btn flex items-center justify-center w-8 h-8 rounded-lg text-[10px] font-black hover:bg-[var(--color-primary)] hover:text-white transition-all text-white/70" data-size="25">25%</button>
        <button type="button" class="img-size-btn flex items-center justify-center w-8 h-8 rounded-lg text-[10px] font-black hover:bg-[var(--color-primary)] hover:text-white transition-all text-white/70" data-size="50">50%</button>
        <button type="button" class="img-size-btn flex items-center justify-center w-8 h-8 rounded-lg text-[10px] font-black hover:bg-[var(--color-primary)] hover:text-white transition-all text-white/70" data-size="75">75%</button>
        <button type="button" class="img-size-btn flex items-center justify-center w-8 h-8 rounded-lg text-[10px] font-black hover:bg-[var(--color-primary)] hover:text-white transition-all text-white/70" data-size="100">100%</button>
      </div>
    </div>
  `.trim();
}
...
export function getBlockAncestor(node: Node | null, container: HTMLElement): HTMLElement | null {
    let curr = node;
    while (curr && curr !== container) {
        if (curr instanceof HTMLElement && ['P', 'H1', 'H2', 'H3', 'BLOCKQUOTE', 'PRE', 'DIV'].includes(curr.tagName)) {
            return curr;
        }
