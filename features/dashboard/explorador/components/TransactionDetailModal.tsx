...
/**
 * The body of the modal for one transaction.
 *
 * A component rather than a fragment because each transaction fetches its own
 * details: the deck mounts this three times during a swipe — the transaction
 * being read plus both neighbours — so what slides in behind the finger is the
 * real transaction, and its details are already on their way.
 */
function TransactionDetailBody({
    tx, onClose, t, dt, copiedAddress, copyAddress, network, timezone, locale, marketData,
}: Pick<TransactionDetailModalProps,
    'tx' | 'onClose' | 't' | 'dt' | 'copiedAddress' | 'copyAddress' | 'network' | 'timezone' | 'locale' | 'marketData'
>) {
...
export function TransactionDetailModal({
    tx, onClose, onPrev, onNext, prevTx, nextTx,
    t, dt, copiedAddress, copyAddress, network, timezone, locale,
    direction = 0, setDirection, marketData,
}: TransactionDetailModalProps) {
    // Everything the body needs except the transaction itself, which is what
    // differs between the current item and the two neighbours.
    const bodyProps = { onClose, t, dt, copiedAddress, copyAddress, network, timezone, locale, marketData };
        ...
        >
            ...
            >
                <SwipeableContainer
                    ...
                    onPrev={onPrev}
                    onNext={onNext}
                    prevContent={prevTx ? <TransactionDetailBody {...bodyProps} tx={prevTx} /> : null}
                    nextContent={nextTx ? <TransactionDetailBody {...bodyProps} tx={nextTx} /> : null}
                    className="flex-1 flex flex-col min-h-0 relative touch-none bg-[var(--color-bg)]"
                >
                    <TransactionDetailBody {...bodyProps} tx={tx} />
                </SwipeableContainer>
            </div>
        </m.div>
    );
