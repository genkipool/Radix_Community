...
export default function AboutRadix({ t }: BaseSectionProps) {
    ...
    const features = [
        { icon: <Target className="w-8 h-8" />, title: t.about.mission.title, desc: t.about.mission.description },
        { icon: <Zap className="w-8 h-8" />, title: t.about.tech.title, desc: t.about.tech.description },
        { icon: <Info className="w-8 h-8" />, title: t.about.team.title, desc: t.about.team.description },
    ];
        ...
        <section id="about" className="py-24 px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto overflow-hidden">
            ...
            <div className="grid md:grid-cols-3 gap-8 mt-16">
                {features.map((feature, i) => (
                    <ScrollReveal key={i} from={{ opacity: 0, y: 20 }} delay={i * 0.1} className="group relative">
