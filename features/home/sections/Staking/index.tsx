      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {(t.staking.stats as Array<Record<string, string>>).map((stat, i: number) => (
            <FadeIn key={i} delay={i * 0.1} className="bg-[var(--color-bg)] border border-[var(--color-card-border)] p-8 rounded-2xl text-center relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-[var(--color-primary)]/5 to-transparent blur-2xl -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-500" />
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
          {(t.staking.features as Array<Record<string, string>>).map((feature, i: number) => (
            <FadeIn key={i} delay={0.3 + i * 0.1} className="group p-8 rounded-2xl bg-[var(--color-bg)] border border-[var(--color-card-border)] hover:border-[var(--color-primary)]/50 transition-all duration-300 shadow-sm hover:shadow-xl">
              <div className="w-12 h-12 rounded-xl bg-[var(--color-surface)] border border-[var(--color-card-border)] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                {i === 0 && <ShieldCheck className="w-6 h-6 text-[var(--color-primary)]" />}
                {i === 1 && <Network className="w-6 h-6 text-[var(--color-secondary)]" />}
                {i === 2 && <Building2 className="w-6 h-6 text-[var(--color-accent)]" />}
              </div>
              {i === 0 ? (
        ...
        <div className="mt-16 bg-gradient-to-br from-[var(--color-primary)]/10 to-transparent border border-[var(--color-secondary)]/30 p-8 rounded-3xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[var(--color-secondary)]/10 rounded-full blur-3xl -mr-20 -mt-20" />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
            ...
            <div className="flex justify-center">
              <div className="w-48 h-48 rounded-full border-4 border-[var(--color-secondary)]/30 flex items-center justify-center relative">
                <div className="absolute inset-0 bg-[var(--color-secondary)]/10 rounded-full animate-pulse" />
                <Droplet className="w-10 h-10 text-[var(--color-secondary)]" />
