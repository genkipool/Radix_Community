...
const PROBLEM_ICONS = [
  <Hourglass key={6} className="w-10 h-10 text-[var(--color-primary)]" />,
  <Unlock key={7} className="w-10 h-10 text-[var(--color-secondary)]" />,
  <Building2 key={8} className="w-10 h-10 text-[var(--color-primary)]" />
];

const SOLUTION_ICONS = [
  <Zap key={12} className="w-10 h-10 text-[var(--color-primary)]" />,
  <Shield key={13} className="w-10 h-10 text-[var(--color-primary)]" />,
  <ClipboardList key={14} className="w-10 h-10 text-[var(--color-primary)]" />
];
    ...
    <section id="problema" className="py-24 bg-[var(--color-bg)] relative overflow-hidden">
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {problems.map((item, i) => (
            <FadeIn key={i} delay={i * 0.1} className="bg-[var(--color-card-bg)] border border-[var(--color-card-border)] p-8 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {solutions.map((item, i) => (
            <FadeIn key={i} delay={i * 0.1} className="bg-gradient-to-br from-[var(--color-primary)]/10 to-transparent border border-[var(--color-secondary)]/30 p-8 rounded-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--color-secondary)]/10 rounded-full blur-2xl -mr-10 -mt-10" />
