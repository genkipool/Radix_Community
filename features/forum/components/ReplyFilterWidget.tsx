    ...
    return (
        <div className="relative filter-dropdown-container">
            <div className={`flex items-center rounded-full border transition-all shadow-md overflow-hidden bg-[var(--color-bg)] border-[var(--color-card-border)] group/filter`}>
                <button onClick={(e) => { e.stopPropagation(); cycleFilterUser('prev'); }}
                ...
                <button onClick={(e) => { e.stopPropagation(); cycleFilterUser('next'); }}
                ...
                {filterDropdownOpen === uniqueFilterId && (
                    ...
                    >
                        ...
                        <div className="max-h-64 overflow-y-auto p-2 custom-scrollbar space-y-1">
                            ...
                            {repliersToAuthor.filter(u => u.name.toLowerCase().includes(filterSearchQuery.toLowerCase())).map(u => (
                                <button key={u.id}
                                    ...
                                    <Image src={u.avatar} alt={u.name} width={24} height={24} className="w-6 h-6 rounded-full object-cover border border-white/20" unoptimized />
