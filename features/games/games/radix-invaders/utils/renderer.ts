...
export function drawBackground(ctx:CanvasRenderingContext2D,state:GameState){
  ...
  for(const st of state.stars){
    ctx.globalAlpha=st.alpha;glow(ctx,'#ffffff',st.r*3);
...
export function drawParticles(ctx:CanvasRenderingContext2D,state:GameState){
  for(const p of state.particles){
    ctx.globalAlpha=p.life;glow(ctx,p.color,p.r*2);
...
export function drawPlayer(ctx:CanvasRenderingContext2D,state:GameState){
  ...
  ctx.globalAlpha=1;
  glow(ctx,COLORS.playerShip,18);ctx.fillStyle=COLORS.playerShip;
  ctx.beginPath();
  ...
  glow(ctx,'#ffffff',10);ctx.fillStyle='rgba(255,255,255,0.7)';
  ...
  ctx.fillStyle=fg;glow(ctx,'#ff8800',16);
  ...
  if(state.powerUp!=='NONE'){
    ...
    ctx.globalAlpha=alpha;glow(ctx,puColor,14);ctx.strokeStyle=puColor;ctx.lineWidth=2;
...
export function drawAliens(ctx:CanvasRenderingContext2D,state:GameState){
  for(const a of state.aliens){
    ...
    if(a.hitFlash>0){ctx.fillStyle='#ffffff';ctx.strokeStyle='#ffffff';glow(ctx,'#ffffff',20);}
    else{ctx.fillStyle=color??COLORS.alien1;ctx.strokeStyle=color??COLORS.alien1;glow(ctx,glowColor??COLORS.glow1,14);}
    ctx.lineWidth=1.5;
...
export function drawUFO(ctx:CanvasRenderingContext2D,state:GameState){
  const{ufo}=state;if(!ufo.active)return;
  ctx.save();ctx.translate(ufo.x,ufo.y);
  const pulse=0.7+0.3*Math.sin(state.tick*0.15);
  glow(ctx,COLORS.ufo,20*pulse);ctx.fillStyle=COLORS.ufo;
  ...
  for(let i=0;i<5;i++){
    ...
    ctx.fillStyle=on?'#ffffff':COLORS.ufo;glow(ctx,on?'#ffffff':COLORS.ufo,on?8:3);
    ctx.beginPath();ctx.arc(lx,UFO_H*0.68,3,0,Math.PI*2);ctx.fill();
  }
  ctx.restore();noGlow(ctx);
}

export function drawShields(ctx:CanvasRenderingContext2D,state:GameState){
  for(const sh of state.shields){
    const{blocks,x,y,blockW,blockH}=sh;
    glow(ctx,COLORS.shield,6);ctx.fillStyle=COLORS.shield;
...
export function drawBullets(ctx:CanvasRenderingContext2D,state:GameState){
  // Player bullets
  for(const b of state.bulletsP){
    ...
    ctx.globalAlpha=1;glow(ctx,color,12);ctx.fillStyle=color;
    if(b.kind==='laser'){
  ...
  for(const b of state.bulletsE){
    ...
    if(b.kind==='banana'){
      ...
      glow(ctx,COLORS.bulletBanana,14);
      ...
      glow(ctx,COLORS.bulletE,10);ctx.strokeStyle=COLORS.bulletE;ctx.lineWidth=2.5;
...
export function drawPowerUpItems(ctx:CanvasRenderingContext2D,state:GameState){
...
export function drawHUD(ctx:CanvasRenderingContext2D,state:GameState,labels:Record<string,string>,mode:'TOURNAMENT'|'FUN'){
  const y=28;
  glow(ctx,COLORS.primary,12);ctx.fillStyle=COLORS.primary;ctx.font='bold 13px "Courier New",monospace';
  ...
  ctx.textAlign='center';ctx.fillStyle=COLORS.gold;glow(ctx,COLORS.gold,10);
  ...
  ctx.textAlign='right';ctx.fillStyle=COLORS.primary;glow(ctx,COLORS.primary,12);
  ...
  ctx.textAlign='left';ctx.fillStyle=COLORS.secondary;glow(ctx,COLORS.secondary,10);
  ...
  if(state.powerUp!=='NONE'){
    ...
    glow(ctx,puColor,8);ctx.fillStyle=`${puColor}44`;ctx.fillRect(CANVAS_W/2-60,CANVAS_H-26,120,8);
  ...
  glow(ctx,mode==='TOURNAMENT'?COLORS.gold:'transparent',8);ctx.font='bold 11px "Courier New",monospace';
...
export function drawGameOver(ctx:CanvasRenderingContext2D,state:GameState,labels:Record<string,string>){
  ctx.save();ctx.fillStyle='rgba(0,0,0,0.72)';ctx.fillRect(0,0,CANVAS_W,CANVAS_H);
  const cx=CANVAS_W/2,cy=CANVAS_H/2;
  if(state.newRecord){glow(ctx,COLORS.gold,30);ctx.fillStyle=COLORS.gold;ctx.font='bold 22px "Courier New",monospace';ctx.textAlign='center';ctx.fillText(labels.new_record??'NEW RECORD',cx,cy-90);}
  glow(ctx,COLORS.secondary,30);ctx.fillStyle=COLORS.secondary;ctx.font='bold 64px "Courier New",monospace';ctx.textAlign='center';ctx.fillText(labels.game_over??'GAME OVER',cx,cy-20);
  ctx.fillStyle=COLORS.white;glow(ctx,COLORS.white,10);ctx.font='18px "Courier New",monospace';ctx.fillText(`${labels.final_score??'SCORE'}: ${state.score}`,cx,cy+36);
  glow(ctx,COLORS.primary,16);ctx.fillStyle=COLORS.primary;ctx.font='bold 16px "Courier New",monospace';ctx.fillText(labels.play_again??'PRESS SPACE TO PLAY AGAIN',cx,cy+80);
  ctx.restore();noGlow(ctx);
}

export function drawStageClear(ctx:CanvasRenderingContext2D,state:GameState,labels:Record<string,string>){
  ctx.save();ctx.fillStyle='rgba(0,0,0,0.5)';ctx.fillRect(0,0,CANVAS_W,CANVAS_H);
  const pulse=0.8+0.2*Math.sin(state.tick*0.1);
  glow(ctx,COLORS.accent,30*pulse);ctx.fillStyle=COLORS.accent;ctx.font='bold 52px "Courier New",monospace';ctx.textAlign='center';
  ...
  if(state.timeBonus>0&&state.timeBonusTimer>0){
    ...
    glow(ctx,COLORS.gold,20);ctx.fillStyle=COLORS.gold;
...
export function drawPaused(ctx:CanvasRenderingContext2D,labels:Record<string,string>){
  ctx.save();ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(0,0,CANVAS_W,CANVAS_H);
  glow(ctx,COLORS.gold,20);ctx.fillStyle=COLORS.gold;ctx.font='bold 52px "Courier New",monospace';ctx.textAlign='center';ctx.fillText(labels.paused??'PAUSED',CANVAS_W/2,CANVAS_H/2-10);
...
export function drawGroundLine(ctx:CanvasRenderingContext2D){
  glow(ctx,COLORS.primary,6);ctx.strokeStyle=COLORS.primary;ctx.lineWidth=1.5;ctx.globalAlpha=0.5;
  ctx.beginPath();ctx.moveTo(0,CANVAS_H-26);ctx.lineTo(CANVAS_W,CANVAS_H-26);ctx.stroke();
  ctx.globalAlpha=1;noGlow(ctx);
}

export function drawFlashMessage(ctx:CanvasRenderingContext2D,state:GameState){
  if(!state.flashMessage||state.flashTimer<=0)return;
  ctx.globalAlpha=Math.min(1,state.flashTimer/30);
  glow(ctx,COLORS.gold,16);ctx.fillStyle=COLORS.gold;ctx.font='bold 20px "Courier New",monospace';ctx.textAlign='center';
  ctx.fillText(state.flashMessage,CANVAS_W/2,90);ctx.globalAlpha=1;noGlow(ctx);
}

export function drawUFOScorePopup(ctx:CanvasRenderingContext2D,state:GameState){
  const p=state.ufoScorePopup;
  if(!p||p.timer<=0)return;
  const alpha=Math.min(1,p.timer/20);
  const rise=(90-p.timer)*0.6;
  ctx.globalAlpha=alpha;
  glow(ctx,COLORS.ufo,14);
