...
const _TOTAL_SPENT = AREAS.reduce((s, a) => s + a.spentBudget, 0);

const _TOTAL_SPENT_USD = AREAS.reduce((s, a) => s + a.spentBudgetUsd, 0);

const _TOTAL_BUDGET = AREAS.reduce((s, a) => s + a.totalBudget, 0);

const _TOTAL_MEMBERS = AREAS.reduce((s, a) => s + a.members.length, 0);

const _TOTAL_TASKS = AREAS.reduce((s, a) => s + a.tasks.length, 0);
...
const _TOTAL_VOLUNTEERS = AREAS.reduce((s, a) => s + a.members.filter(m => m.isVolunteer).length, 0);
