'use client';
/**
 * HeroCarousel — Client island of the Hyperscale hero
 *
 * Same interaction model as the home hero carousel: native CSS animations
 * (animate-hero-in / animate-hero-out from _themes/utilities.css) instead of
 * a motion library, keeping LCP and hydration cost minimal.
 */
...
export default function HeroCarousel({ t }: { t: Dictionary }) {
  ...
  const slides = t.hyperscale.hero.slides;
  ...
  return (
    <>
      <style>{`
        @keyframes hyperscale-hero-progress-fill {
          from { width: 0%; }
          to { width: 100%; }
        }
      `}</style>
      ...
      <div className="grid mt-8">
        {slides.map((slide, idx) => {
          ...
          return (
            ...
            >
              <Heading className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[var(--color-text-main)] mb-4 leading-tight tracking-tighter">
      ...
      >
        <button
          ...
          aria-label={t.hyperscale.hero.btn_prev}
        ...
        {slides.map((slide, index) => (
          <button
            ...
            aria-label={`${t.hyperscale.hero.aria_go} ${index + 1}`}
          >
            <div
              key={`${index}-${activeIdx}`}
              ...
              style={{
                ...
                animation: (activeIdx === index && exitingIdx === null && mounted)
                  ? 'hyperscale-hero-progress-fill 5s linear forwards'
                  : 'none',
              }}
            />
          </button>
        ...
        <button
          ...
          disabled={exitingIdx !== null}
          aria-label={t.hyperscale.hero.btn_next}
          className="group relative flex items-center justify-center size-7 rounded-full border border-[var(--color-card-border)] text-[var(--color-text-muted)] shrink-0 overflow-hidden transition-transform hover:scale-110 active:scale-95 disabled:opacity-50"
        >
          <span className="absolute inset-0 rounded-full bg-[var(--color-text-main)]/6 opacity-0 group-hover:opacity-100 transition-opacity" />
